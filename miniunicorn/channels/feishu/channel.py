...
class FeishuChannel(BaseChannel):
    ...
    def __init__(self, config: Any, bus: MessageBus):
        ...
        self._media: FeishuMediaService | None = None
    ...
    async def login(self, force: bool = False) -> bool:
        ...
        _LOGIN_CONSOLE.print(
            "Authorize with the mobile app. miniunicorn will save the new bot credentials.\n"
        )
    ...
    async def start(self) -> None:
        ...
        self._media = FeishuMediaService(client=self._client, media_dir=get_media_dir("feishu"))
    ...
    # ── Rendering helpers (delegators to pure rendering module) ──────────
    # The pure implementations live in ``miniunicorn.channels.feishu.rendering``
    # so they can be unit-tested without the optional ``lark-oapi`` SDK.
    ...
    def _build_card_elements(self, content: str) -> list[dict]:
        ...
        return build_card_elements(content)
    ...
    def _split_elements_by_table_limit(
        ...
        return split_elements_by_table_limit(elements, max_tables)

    def _split_headings(self, content: str) -> list[dict]:
        """Split content by headings, converting headings to div elements."""
        return split_headings(content)

    @classmethod
    def _detect_msg_format(cls, content: str) -> str:
        """Determine the optimal Feishu message format for *content*."""
        return detect_message_format(content)

    @classmethod
    def _markdown_to_post(cls, content: str) -> str:
        """Convert markdown content to Feishu post message JSON."""
        return markdown_to_post(content)
    ...
    def _upload_image_sync(self, file_path: str) -> str | None:
        ...
        return self._media.upload_image(file_path)

    def _upload_file_sync(self, file_path: str) -> str | None:
        """Upload a file to Feishu and return the file_key."""
        return self._media.upload_file(file_path)

    def _download_image_sync(
        self, message_id: str, image_key: str
    ) -> tuple[bytes | None, str | None]:
        """Download an image from a Feishu message by message_id and image_key."""
        return self._media.download_image(message_id, image_key)

    def _download_file_sync(
        self, message_id: str, file_key: str, resource_type: str = "file"
    ) -> tuple[bytes | None, str | None]:
        """Download a file/audio/media from a Feishu message by message_id and file_key."""
        return self._media.download_file(message_id, file_key, resource_type)
    ...
    async def _download_and_save_media(
        ...
        Returns ``(file_path, content_text)`` — ``file_path`` is None if the
        download failed. The synchronous orchestration lives in
        :class:`FeishuMediaService`; the channel offloads it to a worker thread
        and passes its own overridable ``_download_*_sync`` helpers as the
        download seam so tests can patch them.
        """
        service = FeishuMediaService(
            client=getattr(self, "_client", None), media_dir=get_media_dir("feishu")
        )
        return await asyncio.to_thread(
            service.download_and_save,
            msg_type,
            content_json,
            message_id,
    ...
    def _fallback_text_chunks(text: str, limit: int = 3500) -> list[str]:
        return fallback_text_chunks(text, limit)
    ...
    def _format_tool_hint_lines(tool_hint: str) -> str:
        ...
        return format_tool_hint_lines(tool_hint)

    def _format_tool_hint_delta(self, tool_hint: str) -> str:
        """Format a tool hint string with the 🔧 prefix for each line."""
        return format_tool_hint_delta(tool_hint, self.config.tool_hint_prefix)
