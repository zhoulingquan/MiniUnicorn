...
需要说明：这里的「轻量」指架构哲学与依赖成本——编排核心仅约 3.4k 行、运行时约 30 个纯 Python 依赖；网关默认以监督模式部署（1 个 Control Plane + 3 个 Worker 子进程），轻量单进程模式可用于一次性任务与开发；而完整代码库含频道适配、30 类工具与 WebUI 等外围能力，源码总量约 11 万行，并非「小脚本」级别。
