...
async def test_concurrent_turns_emit_own_context_usage(tmp_path: Path) -> None:
    ...
    tasks = [
        asyncio.create_task(loop._process_message(msg_a, session_key=msg_a.session_key)),
        asyncio.create_task(loop._process_message(msg_b, session_key=msg_b.session_key)),
    ]
