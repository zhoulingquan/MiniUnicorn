...
def test_agent_source_has_no_sqlite3_import() -> None:
    root = Path("miniunicorn/agent")
    violations = []
    # Design exception: the embedding line keeps its provenance-aware
    # sqlite-vec index (vector_index.py + compat shim vector_memory.py) in
    # the agent package. Everything else in agent/ must stay sqlite-free.
    excluded = {"vector_index.py", "vector_memory.py"}
    for path in root.rglob("*.py"):
