...
class TestHandleStop:
    ...
    # Note: test_stop_cancels_active_task and test_stop_cancels_multiple_tasks
    # were removed in design Task 10. The legacy _active_tasks registry was
    # removed; /stop now cancels only in-process subagents. Subagent
    # cancellation is covered by TestSubagentCancellation below.
...
class TestDispatch:
    ...
    @pytest.mark.asyncio
    async def test_process_message_returns_outbound(self):
    ...
    # Note: test_dispatch_streaming_preserves_message_metadata was removed in
    # design Task 10. Streaming is now set up by AgentExecutionCallback, not
    # by the legacy dispatch path. The _wants_stream bus publishing path is
    # covered by runtime/ agent_adapter tests.
