"""W5-1 回归测试:agent/tools → erza/tools 外置(纯搬家)验收。

固化四项验收:
1. 包位置(旧路径不存在、新路径存在);
2. 零残留(源码树下不再出现旧 dotted 路径);
3. 冷导入回归(message/self 作为进程首批 erza 导入,W5-0 循环解除防护);
...
ROOT = Path(erza.__file__).parent

# 拼接构造,避免本测试源文件自身命中被扫描的字符串
_LEGACY_DOTTED = "erza" + ".agent" + ".tools"
...
def test_cold_import_regression() -> None:
    result = subprocess.run(
        [sys.executable, "-c", "import erza.tools.message; import erza.tools.self"],
        check=False,
    )
...
def test_registry_identity_via_new_path() -> None:
    from erza.tools import ToolRegistry, registry
