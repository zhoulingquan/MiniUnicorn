"""Tests for the Erza programmatic facade."""
...
from erza.erza import Erza, RunResult
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        Erza.from_config("/nonexistent/config.json")


def test_from_config_creates_instance(tmp_path):
    config_path = _write_config(tmp_path)
    bot = Erza.from_config(config_path, workspace=tmp_path)
...
def test_from_config_default_path():
    ...
    with (
        patch("erza.config.loader.load_config") as mock_load,
        patch("erza.providers.factory.make_provider") as mock_prov,
    ):
...
def test_workspace_override(tmp_path):
    ...
    bot = Erza.from_config(config_path, workspace=custom_ws)
...
def test_import_from_top_level():
    ...
    assert erza.Erza is Erza
    assert erza.RunResult is RunResult
...
def test_from_config_sets_active_config_path(tmp_path, monkeypatch):
    ...
    from erza.config import loader as loader_mod
    ...
    Erza.from_config(config_path, workspace=tmp_path)
...
def test_from_config_default_path_does_not_override_active_config(tmp_path, monkeypatch):
    ...
    fake_loop = MagicMock()
    monkeypatch.setattr(
        "erza.erza.AgentLoop.from_config",
