...
**裁决:整文件 git mv 至 `erza/providers/model_catalog.py`,cli/models.py 删除。**
...
2. 旧路径 `erza.cli.models` 彻底消失,不留 shim/re-export
...
```powershell
git mv erza/cli/models.py erza/providers/model_catalog.py
```
...
| 文件 | 行 | 改写 |
|---|---|---|
| agent/loop.py | 446/452 | `from erza.cli.models import ...` 改为 `from erza.providers.model_catalog import ...` |
...
| cli/onboard.py | 19-23 | 三符号全部改指 `erza.providers.model_catalog` |
| config/schema.py | 130/194 | **仅注释**:把 `cli.models.get_model_context_limit` 字样更新为 `providers.model_catalog.get_model_context_limit` |
| tests/conftest.py | 26 | `from erza.cli import models as models_module` 改为 `from erza.providers import model_catalog as models_module`(变量名保留以最小化 diff) |
...
禁止 `from erza.cli...` / `import erza.cli...` 两种形式
...
rg -n -e "cli\.models" -e "cli/models" -e "cli import models" erza/ tests/
# 期望零命中(退出码 1)
...
# 门 3:冷导入
.venv\Scripts\python.exe -c "import erza.agent, erza.providers.model_catalog; print('ok')"
...
git log --follow --oneline erza/providers/model_catalog.py   # 期望多条
```
