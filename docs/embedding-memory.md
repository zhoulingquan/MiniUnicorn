...
```bash
python scripts/verify_embedding_memory.py
```

脚本会使用真实的 FastEmbed + sqlite-vec 执行 13 项检查，包括模型加载、中文向量质量、索引持久化、查询排序、增量同步、删除重建、取消保护、指纹校验、结果预算、来源追溯和聊天回退。成功时 stdout 最后一行为 `{"ok":true,...}`，失败退出码为 1。
