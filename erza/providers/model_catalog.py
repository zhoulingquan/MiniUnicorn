...
1. Looking up a permanent learning table
   (``.erza-config/cache/model_context_learned.json``) — entries are
...
ENV_NO_AUTO_LOOKUP = "ERZA_NO_AUTO_LOOKUP"
    ...
    if os.environ.get(ENV_NO_AUTO_LOOKUP):
        if raise_on_unknown:
            raise RuntimeError(
                ...
                f"请在 .erza-config/config.json 的 model 段显式设置 "
    ...
    try:
        ...
        if raise_on_unknown:
            raise RuntimeError(
                ...
                f"  原因: {exc}\n"
                f"  解决方法: 在 .erza-config/config.json 的 model 段显式设置 "
                f"'context_window_tokens' 字段(如 128000),\n"
...
def _get_learning_table_path() -> Path:
    ...
    try:
        ...
        return Path.home() / ".erza-cache" / "model_context_learned.json"
