---
name: update-setup
description: One-time setup wizard for the Erza upgrade skill. Triggers: setup update, configure update, 切设置更新, 初始化更新.
---
...
Use `exec` to run `erza --version`. Tell the user the current version.
...
```
command -v Erza || true
python -m pip show erza-ai || true
pipx list | sed -n '/erza-ai/,+3p' || true
uv tool list | sed -n '/erza-ai/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how Erza was
installed.
...
`question: "Where is your Erza source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "erza-ai[EXTRAS]" --force` |
| pipx | `pipx install --force "erza-ai[EXTRAS]"` |
| pip | `python -m pip install --upgrade "erza-ai[EXTRAS]"` |
...
---
name: update
description: "Upgrade Erza to the latest version. Triggers: upgrade Erza, update Erza, 升级Erza, 更新Erza."
---
...
4. Use `exec` to verify: `erza --version`
5. Tell the user the new version. Say: "Run `/restart` to restart Erza and apply the update. If `/restart` is unavailable in this channel, restart the Erza process manually."
```

## Step 5: Confirm

Tell the user: "Upgrade skill created. Say 'upgrade Erza' when you want to update."
