...
``erza.cli.commands`` module namespace (e.g.
...
``patch("erza.cli.commands._PROMPT_SESSION", ...)`` continues to
...
def _init_prompt_session() -> None:
    ...
    so that ``patch("erza.cli.commands.PromptSession", ...)``
    takes effect.
    """
    from erza.cli import commands
    ...
    from erza.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    if show_header:
        console.print()
        console.print(f"[cyan]{__logo__} Erza[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} Erza[/cyan]"),
...
def _flush_cli_reasoning(
    ...
    ``patch("erza.cli.commands._print_cli_reasoning", ...)`` continues
...
async def _print_interactive_progress_line(
    ...
    that ``patch("erza.cli.commands._print_interactive_line", ...)``
...
async def _maybe_print_interactive_progress(
    ...
    the ``commands`` module so that ``patch("erza.cli.commands.<name>",
    ...)`` continues to take effect after the split.
    """
    from erza.cli import commands
...
async def _read_interactive_input_async() -> str:
    ...
    ``patch("erza.cli.commands._PROMPT_SESSION", ...)`` /
    ``patch("erza.cli.commands.patch_stdout", ...)`` continue to
