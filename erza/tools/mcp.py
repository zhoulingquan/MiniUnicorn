"""MCP client: connects to MCP servers and wraps their tools as native Erza tools."""
...
def _validate_mcp_url(url: str) -> str:
    ...
    from erza.security.network import _is_metadata_blocked
...
class MCPToolWrapper(Tool):
    """Wraps a single MCP server tool as a Erza Tool."""
...
class MCPResourceWrapper(Tool):
    """Wraps an MCP resource URI as a read-only Erza Tool."""
...
class MCPPromptWrapper(Tool):
    """Wraps an MCP prompt as a read-only Erza Tool."""
    ...
    for item in structured[:8]:
        ...
        if connected_server_names is not None and raw_name not in connected_server_names:
            lines.append(
                ...
                "run the preset test, and restart Erza only if hot reload is unavailable."
            )
            continue
        lines.append(
            "MCP Preset Attachment: "
...
async def reload_servers(state: RuntimeState, registry: ToolRegistry) -> dict[str, Any]:
    ...
    async with _reload_lock(state):
        try:
            from erza.config.loader import load_config, resolve_config_env_vars
            ...
            return {
                "ok": False,
                "message": "Could not reload MCP config. Restart Erza to pick up changes.",
        ...
        if failed:
            ...
            message = "MCP connections refreshed without restarting Erza."
        else:
            message = "MCP config reloaded without restarting Erza."
...
async def request_mcp_reload(bus: Any, *, timeout: float = 15.0) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "ok": False,
            "message": "MCP hot reload timed out. Restart Erza to pick up changes.",
            "requires_restart": True,
        }
    return (
        result
...
async def handle_runtime_control(
    ...
    try:
        ...
        result = {
            "ok": False,
            "message": "MCP hot reload failed. Restart Erza to pick up changes.",
            "requires_restart": True,
            "error": str(exc),
        }
