...
- Publish to erza bus via BaseChannel._handle_message()
...
# _IMAGE_EXTS / _sanitize_filename 由 erza.channels._media_common 提供，
...
def _make_bot_class(channel: QQChannel) -> type[botpy.Client]:
    ...
    class _Bot(botpy.Client):
        def __init__(self):
            # Disable botpy's file log — erza uses loguru; default "botpy.log" fails on read-only fs
...
class QQChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if not QQ_AVAILABLE:
            self.logger.error("SDK not installed. Run: erza plugins enable qq")
            return
