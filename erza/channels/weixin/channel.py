...
from erza.channels._media_common import _IMAGE_EXTS
from erza.channels._media_common import _VIDEO_EXTS as _BASE_VIDEO_EXTS
from erza.channels.base import BaseChannel
from erza.config.paths import get_media_dir, get_runtime_subdir
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                self.logger.error(
                    "login failed. Run 'erza channels login weixin' to authenticate."
                )
                self._running = False
                return
